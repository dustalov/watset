...
for ISAS in {patterns,patterns-limit,wiktionary,mas,joint}-filter{,-exp}-isas.txt; do
  if [ ! -f "$ISAS" ]; then
    touch "$ISAS"
  fi
...
./evaluate.py --gold=ruthes-isas.txt {patterns,patterns-limit,wiktionary,mas,joint}-filter{,-exp}-isas.txt $EVALUATE | tee pairwise-ruthes.tsv | sort -t $'\t' -g -k9r | column -t
